...
| 21 | `catalog-row-length` | WARN | A single entry in `catalog.md` longer than `catalog_row_max_chars` — an entry says what a file is and why to open it, and this one grew into a summary of the file. The counterpart of #17 for the other file read on every entry into an entity, and deliberately the **same threshold value**: the two mean the same thing, and two thresholds with one meaning and two values are two thresholds somebody eventually confuses. Every entry shape counts, because real catalogs are not uniform — a bullet naming its file in a code span (`` - `file` — what it is ``), in a link, or in bold (`- **file.md** — what it is`), and a table row (`| question | file |`); ignoring the table form would under-count by about a third, and ignoring the bold form let an entry of any length pass unmeasured. Bold alone is not the test — a catalog also carries prose bullets opening on an emphasised phrase, so the bold run has to end the way a filename or a directory name ends. An entry is measured as a **logical item**, with the lines it was wrapped onto folded in: a hard wrap is typographic, and measuring the first physical line alone let an entry of 2 000 characters pass a 1 000-character threshold as long as its author pressed Enter — seven such entries were invisible in the repository this check was calibrated on. A nested list item is not a continuation but an entry in its own right, so a bullet grouping eleven short ones is not charged their combined length; folding stops at a blank line too, which under-counts a multi-paragraph item — the safe direction for a threshold. A **WARN**, unlike #22: shortening is editorial, the entry is sometimes the only description a file has, and it cannot be repaired without a human deciding what to keep — so while the repository still carries a backlog of them, an ERROR would make the linter permanently red and hand `/close-session` scopes the session never touched. The level rises after the debt is paid, the road #19 took; here that means the entities on `catalog_row_length_exclude` being cleared. That list holds entities skipped whatever their entries look like, each **with the reason it is exempt** — a required field, because an exception without a reason is indistinguishable from an oversight and nobody can tell later whether it still holds. Empty in the template: an entity name is a fact about a repository, never about the template it grew from. One finding per entry, capped at five per file with the remainder summarised in one line. |
| 22 | `index-log` | **ERROR** | An `_index.md` carrying a section with the history of its own changes (`## Recent Changes`, `## Historia zmian`, `## Changelog`, …) — it belongs in `_changelog.md` beside the index. The index is read on every entry into the scope; its own log answers a question almost nobody asks at that moment, so left inside it grows without bound and is paid for on every read. Measured before this check existed: `context/projects/_index.md` was 83 kB, of which 78 kB was its own log — with the rule against it already written in that file's header. A sentence in a header is not a guard. Matched against the **whole heading**, never as a substring: an index may legitimately catalogue a `change-orders/` directory or a `decision-log.md` file, and a substring match would call those a changelog. Code blocks are skipped, for the same reason at a higher stake: inside one the text is an example of a heading, not a heading, and at ERROR level that difference decides whether an index documenting this very rule passes or fails. Both shapes count — a fence, tracked by its own character and run length so a four-backtick block showing a plain fence inside it stays one block, and four spaces of indentation, which is what CommonMark makes of a line that deep. An **ERROR** from its first day, unlike #19 and #21: the repair is mechanical, lossless and has one known destination, and the entire backlog (five files) was paid in the same change that added the check — so the level never stood over an existing debt. Runs once over every directory the config declares (scan roots, file scopes, the task registry, and anything listed in `index_log_roots`), because a nested `_index.md` deep inside `data/` is as much an index as the scope root's — and because an index can sit outside every declared scope, where the guard would otherwise be silently blind. One finding per file: the fix is the whole section, not a line of it. |
| 23 | `verified-shape` | **ERROR** | A `verified` entry that is present but malformed — an entry with no `by`, an actor carrying no recognised kind (`human:<id>` for a person, `process:<id>` for an automated process, `<vendor>/<version>` for a tool or an agent), an entry with no `at`, or an `at` that is not ISO 8601 with an explicit UTC offset. **The absence of the field is never reported**, and that silence is a required property rather than a simplification: "unconfirmed" is the base's default state and not a debt to pay off, so warning about it would put a finding on every file at once — measured on the repository this check landed in, 955 files sat in scope and not one carried the field, which is exactly the run that would have turned into 955 findings and taught everyone to ignore the check inside a week. What is checked is only the entry somebody deliberately wrote, because that is the one place a mistake can hide behind the appearance of a confirmation. A header that carries a `verified` key and does **not** parse as YAML is reported all the same, and that is not an exception to the silence: the silence is owed to a field nobody wrote, never to one written and then made unreadable — an unterminated list under `verified` arrives at the check looking exactly like a file that never had the field, which is the one way a malformed trace could otherwise walk past the check that exists for it. That fallback reads the raw header rather than a parse of it, so it is deliberately generous about where the key may sit: at the start of a line, quoted, or inside a flow mapping, and in a fence that opens and never closes as much as in one that closes. Detecting the key structurally is not on the table there, since the branch runs precisely because nothing could parse the header. The offset is required because the value earns its keep by being comparable with the file's last change, and a wall-clock time with no zone is ambiguous by an hour twice a year; YAML resolves a well-formed timestamp to a datetime and a bare date to a date, so both typed shapes are judged and a string survives only when it parsed as nothing else — which is the malformed case. A single entry written as a bare mapping with no list dash is read as a one-item list, never rejected on punctuation. An **ERROR** from its first day, the road #22 took and #19 could not: every failure has one mechanical repair — add the key, prefix the actor, rewrite the timestamp — and there was no backlog to stand over, since no file carried the field when the check landed. What the check cannot do is judge whether a `human:` entry is truthful: an entry an agent awarded itself is byte-identical to one written after the person confirmed, which is why that prohibition lives in the doctrine and not here, and why this check is deliberately about shape alone. Scope comes from `verified_scope` in `config.yaml` — directory names matched at any depth, so a nested engagement's `data/` counts as much as the entity's own — and subtrees on its `exclude` map are skipped **only when the entry carries its reason**, the same bargain #21 strikes for the same reason. Material that is put down outranks the directory name: a `data/` sitting under `archive/`, `communication/`, `output/`, `inbox/` or `tasks/` is out of scope however it is named, or moving a file into the archive together with its `data/` would be enough to make it confirmable somewhere the check never looks. That list is doctrine rather than repository data, so it lives in the check as `VERIFIED_PUT_DOWN_DIRS` and the confirmation command reads it from there instead of keeping a copy. Empty in the template: which directories hold settled facts is a fact about a repository, never about the template it grew from, and an unconfigured check is skipped in silence. |
...
- `file_scopes` — file-shaped scopes: check #2, and the tree check #22 walks
  looking for `_index.md`.
...
- `catalog_row_length_exclude` — entities skipped by check #21, mapped to the reason
  each is exempt. Empty in the template; a reason is required, never optional — an
  entry with a blank one is reported and the entity is checked anyway.
- `index_log_roots` — further trees check #22 walks looking for `_index.md`, beyond
  the declared scopes and the task registry. Empty in the template: which trees a
  repository keeps indexes in is a fact about that repository.
